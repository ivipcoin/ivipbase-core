...
export class DataSnapshotsArray<T = any> extends Array<DataSnapshot<T>> {
	constructor(length: number, readonly isMore: boolean) {
		super(length);
	}
	static from<T = any>(snaps: DataSnapshot<T>[], isMore: boolean): DataSnapshotsArray<T>;
	static from<T = any>(snaps: DataSnapshot<T>[]): T[];
	static from<T, U>(snaps: DataSnapshot<T>[], mapfn: (v: T, k: number) => U, thisArg?: any): U[];
	static from<T = any>(snaps: DataSnapshot<T>[], arg2?: any): any {
		const isMore: boolean = typeof arg2 === "boolean" ? arg2 : false;
		const arr = new DataSnapshotsArray<T>(snaps.length, isMore);
...
export class DataReferencesArray<T = any> extends Array<DataReference<T>> {
	...
	static from<T = any>(refs: DataReference<T>[], isMore: boolean): DataReferencesArray<T>;
	static from<T = any>(refs: DataReference<T>[]): T[];
	static from<T, U>(refs: DataReference<T>[], mapfn: (v: T, k: number) => U, thisArg?: any): U[];
	static from<T = any>(refs: DataReference<T>[], arg2?: any): any {
		const isMore: boolean = typeof arg2 === "boolean" ? arg2 : false;
		const arr = new DataReferencesArray<T>(refs.length, isMore);
...
export class DataReferenceQuery {
	...
	get<T = any>(
		...
		return db.storage
			...
			.then((res) => {
				const { stop } = res;
				let { results, context, isMore } = res;
				this.stop = async () => {
					await stop();
				};
				...
				if (options.snapshots) {
					...
					return DataSnapshotsArray.from(snaps, isMore);
				} else {
					const refs = (results as string[]).map<DataReference>((path) => db.ref(path));
					return DataReferencesArray.from(refs, isMore);
				}
			})
			.then((results) => {
